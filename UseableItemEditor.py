...
class UseableItemDataEditor:
    """
    Useable Item Statistics Editor.
    """
    def __init__(self, parent, save_manager=None):
        ...
        self.USEABLE_MAP = {
            "value": "USEABLE_VALUE",
            "weight": "USEABLE_WEIGHT",
            "dam_min": "USEABLE_MIN_DMG",
            "dam_max": "USEABLE_MAX_DMG",
            "max_durability": "USEABLE_MAX_DURABILITY",
            "use_type": "USEABLE_USE_TYPE",
            "req_lvl": "USEABLE_REQ_LEVEL",
            "req_str": "USEABLE_REQ_STR",
            "req_int": "USEABLE_REQ_INT",
            "req_dex": "USEABLE_REQ_DEX",
            "req_con": "USEABLE_REQ_CON"
        }
        
        self.win = tk.Toplevel(parent)
        self.win.title("Useable Item Data")
    ...
    def setup_ui(self):
        ...
        tk.Label(top_f, text="Useable Item Type:", bg="#dfdfdf", font=self.title_font).pack(side="left")
    ...
    def refresh_type_dropdown(self):
        ...
        items = sorted([f"{tid}: {v.get('name', 'Unnamed')}" for tid, v in self.types_data.items() if v.get("family") == "FAM_USEABLE"], key=lambda x: x.split(":")[1].lower())
    ...
    def load_type_data(self):
        ...
        for k, var in self.stat_vars.items():
            var.set(hairy_data.get(self.USEABLE_MAP.get(k), "0"))
        
        # Load use type
        ut_idx = int(hairy_data.get("USEABLE_USE_TYPE", "0"))
        ...
        self.win.title(f"Useable Item Data - {data.get('name', 'Unnamed')}")
    ...
    def apply(self):
        ...
        try:
            ...
            hairy_path = os.path.join(self.save_manager.project_path, "HAIRY", hairy_filename)
            hairy_props = {self.USEABLE_MAP[k]: int(v.get() or 0) for k, v in self.stat_vars.items() if k in self.USEABLE_MAP}
            
            ut_list = ["NONE", "MINE", "SMELT", "FORGE", "BOOST", "REPAIR", "TELEPORT", "TELEP_AND_SPAWN", "SPAWN_GATE"]
            use_type_val = ut_list.index(self.use_type_var.get()) if self.use_type_var.get() in ut_list else 0
            hairy_props["USEABLE_USE_TYPE"] = use_type_val
            hairy_props["FAM_USEABLE"] = 1
